...
void ui_add(contacts_list** list)
{
    ...
    int new_id = contacts_list_append(list, new_info);
    if (new_id < 0)
    {
    ...
    contacts_list_print_id(*list, new_id);
}
...
void ui_delete(contacts_list** list)
{
    ...
    int id = atoi(index_str);
    int result = contacts_list_remove(list, id);
    if (check_if_fail(result, CONTACTS_LIST_OK, "Error: id not found.") == UI_SIGNAL_INPUT_ERROR)
        return;
