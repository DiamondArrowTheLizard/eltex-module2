...
void ui_add(array_contacts* array)
{
    ...
    array_contacts_print_at_index(array, array->count-1);
...
void ui_delete(array_contacts* array)
{
    ...
    index = atoi(index_str);
    int valid_request = array_contacts_remove(array, index);
    if(check_if_fail(valid_request, ARRAY_CONTACTS_OK, "Error: index out of range.") == UI_SIGNAL_INPUT_ERROR) return;
...
void ui_print_entry(array_contacts* array)
{
    ...
    int valid_request = array_contacts_print_at_index(array, index);
    if(check_if_fail(valid_request, ARRAY_CONTACTS_OK, "Error: index out of range.") == UI_SIGNAL_INPUT_ERROR) return;
